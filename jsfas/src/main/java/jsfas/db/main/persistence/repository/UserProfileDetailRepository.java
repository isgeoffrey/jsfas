...
public interface UserProfileDetailRepository extends JpaRepository<UserProfileDetailDAO, UserProfileDetailDAOPK> {
	...
	@Query(value = "SELECT * FROM FAS_USER_PROFILE_DTL WHERE USER_NAM = :userName "
	...
	@Query(value = "SELECT * FROM FAS_USER_PROFILE_DTL WHERE "
	...
	@Query(value = "SELECT * FROM FAS_USER_PROFILE_DTL WHERE USER_NAM = :userName ", nativeQuery = true)
	List<UserProfileDetailDAO> findByUserName(
			@Param("userName") String userName); 
	
	@Modifying
	@Query(value = "INSERT INTO FAS_USER_PROFILE_DTL "
			+ "SELECT "
			...
			+ "FROM FAS_FUNC_TAB F "
			+ "LEFT OUTER JOIN FAS_USER_PROFILE_DTL D "
			...
			+ "AND F.FUNC_CDE IN (SELECT DISTINCT ACCESS_FUNC_CDE FROM FAS_USER_PROFILE_DTL WHERE USER_NAM = :userName) ", nativeQuery = true)
