...
public interface FunctionRepository extends JpaRepository<FunctionDAO, FunctionDAOPK> {
	...
	@Query(value = "SELECT * FROM FAS_FUNC_TAB "
			+ " WHERE SYS_CATG_CDE = :sysCatgCode "
			+ " AND FUNC_CATG_CDE = :funcCatgCode "
			+ " AND FUNC_CDE IN (SELECT ACCESS_FUNC_CDE FROM FAS_USER_PROFILE_DTL WHERE USER_NAM = :userName)", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM FAS_FUNC_TAB "
			...
			+ " AND ((FUNC_CDE,FUNC_SUB_CDE) IN (SELECT ACCESS_FUNC_CDE,ACCESS_FUNC_SUB_CDE FROM FAS_USER_PROFILE_DTL WHERE USER_NAM = :userName) "
			+ " or (FUNC_CDE, FUNC_SUB_CDE) IN (SELECT ACCESS_FUNC_CDE,' ' as ACCESS_FUNC_SUB_CDE FROM FAS_USER_PROFILE_DTL WHERE USER_NAM = :userName))", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM FAS_FUNC_TAB WHERE SYS_CATG_CDE = :sysCatgCode "
			+ "AND MENU_GEN_IND = 'Y' "
			+ "AND FUNC_CDE IN (SELECT ACCESS_FUNC_CDE FROFAS_USER_PROFILE_DTL WHERE USER_NAM = :userName)", nativeQuery = true)
